...
export function MicButton({ onTranscript, className = '' }) {
  ...
  useEffect(() => {
    const checkSupport = () => {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        setError('マイクがサポートされていません。HTTPSまたはモダンブラウザをご使用ください。');
        return;
      }
      ...
      if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
        setIsSupported(false);
        setError('マイクはセHTTPSが必要です。安全な接続をご使用ください。');
        return;
      }
  ...
  const startRecording = async () => {
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error('マイクアクセスが利用できません。HTTPSまたはサポートされたブラウザをご使用ください。');
      }
      ...
      if (err.name === 'NotAllowedError') {
        errorMessage = 'マイクアクセスが拒否されました。マイクのアクセスを許可してください。';
      } else if (err.name === 'NotFoundError') {
        errorMessage = 'マイクが見つかりません。オーディオデバイスを確認してください。';
