                          ...
                          <div className="mt-3 space-y-2">
                            <div className="bg-gray-900 dark:bg-gray-950 text-gray-100 rounded-lg p-3 font-mono text-sm">
                              <div className="flex items-center gap-2 mb-2 text-gray-400">
                                ...
                                <span className="text-xs">ターミナル</span>
                              </div>
                  ...
                  <div className="mt-3 border-t border-blue-200 dark:border-blue-700 pt-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-sm font-medium ${
                        ...
                        {message.toolResult.isError ? 'ツールエラー' : 'ツール結果'}
                      </span>
                    </div>
                    ...
                    <div className={`text-sm ${
                      ...
                      {(() => {
                        ...
                        if ((message.toolName === 'TodoWrite' || message.toolName === 'TodoRead') &&
                            (content.includes('Todos have been modified successfully') || content.includes('Todoリストが正常に更新されました') || 
                          ...
                          try {
                            ...
                            } else if (content.includes('Todos have been modified successfully') || content.includes('Todoリストが正常に更新されました')) {
                              // For TodoWrite success messages, we don't have the data in the result
                              return (
                                <div>
                                  <div className="flex items-center gap-2 mb-2">
                                    <span className="font-medium">Todoリストが正常に更新されました</span>
                                  </div>
                                </div>
                              );
                            }
                              ...
                              return (
                                <div>
                                  <div className="flex items-center gap-2 mb-3">
                                    <span className="font-medium">現在のTodoリスト</span>
                                  </div>
                          ...
                          try {
                            ...
                            if (parsed.plan) {
                              ...
                              return (
                                <div>
                                  <div className="flex items-center gap-2 mb-3">
                                    <span className="font-medium">実装計画</span>
                                  </div>
                        ...
                        if ((content.includes('Do you want to proceed?') || content.includes('続行しますか？')) && message.toolName === 'Bash') {
                          const lines = content.split('\n');
                          const promptIndex = lines.findIndex(line => line.includes('Do you want to proceed?') || line.includes('続行しますか？'));
                          const beforePrompt = lines.slice(0, promptIndex).join('\n');
                          const promptLines = lines.slice(promptIndex);
                          
                          // Extract the question and options
                          const questionLine = promptLines.find(line => line.includes('Do you want to proceed?') || line.includes('続行しますか？')) || '';
                          const options = [];
                                    ...
                                    {selectedOption && (
                                      <div className="bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3">
                                        <p className="text-amber-900 dark:text-amber-100 text-sm font-medium mb-1">
                                          ✓ Claudeがオプション{selectedOption}を選択
                                        </p>
                                        <p className="text-amber-800 dark:text-amber-200 text-xs">
                                          CLIでは、矢印キーを使うか番号を入力してこのオプションを選択できます。
                                        </p>
                                      </div>
                                    )}
                                  </div>
                        ...
                        if (fileEditMatch) {
                          return (
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                <span className="font-medium">ファイルが正常に更新されました</span>
                              </div>
                              <button 
                        ...
                        if (fileCreateMatch) {
                          return (
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                <span className="font-medium">ファイルが正常に作成されました</span>
                              </div>
                              <button 
                          ...
                          return (
                            <div className="text-green-700 dark:text-green-300">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">ファイルが正常に書き込まれました</span>
                              </div>
                              <p className="text-xs mt-1 text-green-600 dark:text-green-400">
                                ファイルの内容は上記の差分表示に表示されています
                              </p>
                            </div>
                          );
                        }
                      ...
                      return (
                        <>
                          ...
                          <div className="bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3">
                            ...
                            <p className="text-amber-800 dark:text-amber-200 text-xs">
                              Claudeが実行されているターミナルでオプションを選択してください。
                            </p>
                          </div>
                        </>
                      );
...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, messages, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onReplaceTemporarySession, onNavigateToSession, onShowSettings, autoExpandTools, showRawParameters, autoScrollToBottom }) {
  ...
  const convertedMessages = useMemo(() => {
    console.log(`[ChatInterface] Converting ${sessionMessages.length} session messages`);
    const converted = convertSessionMessages(sessionMessages);
    console.log(`[ChatInterface] Converted to ${converted.length} messages:`, converted);
    return converted;
  }, [sessionMessages]);
  ...
  const handleImageFiles = useCallback((files) => {
    const validFiles = files.filter(file => {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setImageErrors(prev => new Map(prev).set(file.name, 'ファイルサイズが大きすぎます (最大5MB)'));
        return false;
      }
      return true;
    });
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p>Claudeとチャットを開始するプロジェクトを選択してください</p>
        </div>
      </div>
    );
  }
          ...
          <>
            {chatMessages.length > visibleMessageCount && (
              <div className="text-center text-gray-500 dark:text-gray-400 text-sm py-2 border-b border-gray-200 dark:border-gray-700">
                最新{visibleMessageCount}メッセージを表示 (合計{chatMessages.length}) • 
                <button 
        ...
        <div className="max-w-4xl mx-auto mb-3">
          <div className="flex items-center justify-center gap-3">
            <button
              ...
              title="権限モードを変更するにはクリック（または入力でTabを押す）"
            >
              <div className="flex items-center gap-2">
                ...
                <span>
                  {permissionMode === 'default' && 'デフォルトモード'}
                  {permissionMode === 'acceptEdits' && '編集を承認'}
                  {permissionMode === 'bypassPermissions' && '権限をバイパス'}
                  {permissionMode === 'plan' && '計画モード'}
                </span>
              </div>
            </button>
        ...
        <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
          ...
          <div {...getRootProps()} className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 ${isTextareaExpanded ? 'chat-input-expanded' : ''}`}>
            ...
            <textarea
              ...
              placeholder="コードのヘルプをClaudeに依頼... (@ でファイルを参照)"
              disabled={isLoading}
              rows={1}
