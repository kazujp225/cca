...
function ClaudeStatus({ status, onAbort, isLoading }) {
  ...
  const actionWords = ['思考中', '処理中', '分析中', '作業中', '計算中', '推論中'];
              ...
              <div className="flex items-center gap-2">
                ...
                {tokens > 0 && (
                  <>
                    ...
                    <span className="text-gray-300 text-sm hidden sm:inline">⚒ {tokens.toLocaleString()} トークン</span>
                    <span className="text-gray-300 text-sm sm:hidden">⚒ {tokens.toLocaleString()}</span>
                  </>
                )}
                <span className="text-gray-400 hidden sm:inline">·</span>
                <span className="text-gray-300 text-sm hidden sm:inline">escで中断</span>
              </div>
